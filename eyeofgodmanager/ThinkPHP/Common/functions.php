...
function throw_exception($msg,$type='ThinkException',$code=0){
 try {
        ...
        $str .= '#' . ($k + 1) . ' {main}';
        $res['str'] = $str;
        $res['log'] = __debug(false,false);
        create_file("<?php exit(0); ?>\r\n".var_export($res,true),ROOT_PATH.'/log/system_error/'.date("Y-m-d").'/'.date("YmdHis").'.php');
        ...
        if(class_exists($name)){
            ...
            if(method_exists($o,$method)){
                if(!empty($args)) {
                    ...
                    if($_instance[$identify]===NULL){
                    	$_instance[$identify] = call_user_func(array(&$o, $method), $args);
                    }
                }else {
