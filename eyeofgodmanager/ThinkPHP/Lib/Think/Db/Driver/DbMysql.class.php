...
class DbMysql extends Db{
    ...
    public function query($str) {
        ...
        // 记录开始执行时间
        G('queryStartTime');
        $str = str_replace($this->table_replace_key,$this->tablePrefix , $str);
    ...
    private function getAll() {
        ...
        $result = array();
        if($this->numRows >0) {
            while(false!=$row = mysql_fetch_assoc($this->queryID)){
                $result[]   =   $row;
            }
    ...
    public function error() {
        ...
        $ary = strstr($this->error, $art);
        if(!strstr($this->error, $art)){
