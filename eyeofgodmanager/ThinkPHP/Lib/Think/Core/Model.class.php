...
class Model extends Think
{
    ...
    protected $table_replace_key = '';
    protected $connect_new_db= false;
    ...
    public function __construct($name='',$connection='')
    {
        ...
        // 获取数据库操作对象
        // 当前模型有独立的数据库连接信息
       $_ENV['__db__'] =  $this->db = Db::getInstance(empty($this->connection)?$connection:$this->connection);
        // 设置表前缀
        if(!$this->connect_new_db){
        	$this->tablePrefix = $this->tablePrefix?$this->tablePrefix:C('DB_PREFIX');
        }
        $this->tableSuffix = $this->tableSuffix?$this->tableSuffix:C('DB_SUFFIX');
        $this->table_replace_key = $this->table_replace_key?$this->table_replace_key:C('DB_PREFIX_REPLACE_KEY');
        // 字段检测
    ...
    private function _parseOptions($options) {
        ...
        if($options){
	        foreach ($options as $key=>&$item){
	        	if(is_array($item)){
	        		foreach ($item as &$val){
	        			$val = str_replace($this->table_replace_key,$this->tablePrefix , $val);
	        		}
	        	}else{
	        		$item = str_replace($this->table_replace_key,$this->tablePrefix , $item);
	        	}
	        }
        }
        return $options;
     ...
     * @return mixed
     +----------------------------------------------------------
     */
    public function query($sql,$one = false,$object = false){
    	if(!empty($sql)) {
            if(strpos($sql,'__TABLE__'))$sql    =   str_replace('__TABLE__',$this->getTableName(),$sql);
            $sql = str_replace($this->table_replace_key,$this->tablePrefix , $sql);
            $data = $this->db->query($sql);
            $data = empty($data)?array():$data;
            if($data && $one){
            	$ee = current($data);
            	return $object?array_to_object($ee):$ee;
            }
    ...
    function fetch($sql,$one = false, $object = false){
    	return $this->query($sql,$one,$object);
    }
