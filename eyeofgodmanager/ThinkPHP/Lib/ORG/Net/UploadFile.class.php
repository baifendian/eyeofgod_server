    ...
    */
    ...
    private function save($file)
    {
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
               ...
               /* // 生成图片水印
                /*$gConfig = getContent('cms.config.php', './');
                if($gConfig && $gConfig['watermark_status'] == 1){
                    Image::waterMark($filename, $gConfig['watermark_size'], $gConfig['watermark_position'], $gConfig['watermark_padding'], $gConfig['watermark_trans'], $this->waterMarkFile);
                }*/
    ...
    private function check($file) {
        ...
        if(!$this->checkExt($file['extension'])) {
            $this->error = '上传文件类型不允许。允许类型：'.implode(',',$this->allowExts);//debug
            return false;
        }
